...
//     return await apiFetch<SubCategoriesResponse>(`/subcategories?${params}`, {
//       cache: 'force-cache',
...
//   try {
//     return await apiFetch<SubCategory>(`/subcategories/${id}`, {
//       cache: 'force-cache',
...
export async function fetchSubCategories(
  ...
  try {
    ...
    // CORRECT ENDPOINT: /api/v1/sub_categories
    return await apiFetch<SubCategoriesResponse>(`/sub_categories?${params}`, {
      cache: 'force-cache',
      next: { revalidate: 3600 }
    });
  } catch (error) {
...
export async function fetchSubCategoryById(id: number): Promise<SubCategory | null> {
  try {
    // CORRECT ENDPOINT: /api/v1/sub_categories/{id}
    return await apiFetch<SubCategory>(`/sub_categories/${id}`, {
      cache: 'force-cache',
      next: { revalidate: 3600 }
    });
  } catch (error) {
