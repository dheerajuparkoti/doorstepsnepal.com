...
interface MenuItem {
  ...
  icon: React.ComponentType<{ className?: string }>;
  children?: MenuItem[];
  verifiedOnly?: boolean; // Added this property
}
...
export function MobileBottomNav() {
  ...
  const pathname = usePathname();
  const router = useRouter();
  const { mode, user } = useAuth(); // Destructured user here
  const [openMenu, setOpenMenu] = useState<string | null>(null);
  const [openSections, setOpenSections] = useState<string[]>([]);
  ...
  const customerMenuItems: MenuItem[] = [
    ...
    { label: t.nav.profile, href: "/dashboard/profile/customer", icon: User },
    {
      label: t.customer.sidebar.payments,
      href: "/dashboard/payments",
      icon: CreditCard,
    ...
    },
    {
      ...
      children: [
        ...
        { 
          ...
          icon: AwardIcon,
          verifiedOnly: true // Logic applied here
        },
      ],
    },
    {
  ...
  const fullMenu = mode === "customer" ? customerMenuItems : professionalMenuItems;

  // Filter items: Remove any top-level or child item if verifiedOnly is true and user is not verified
  const filteredMenu = fullMenu.filter(item => !item.verifiedOnly || user?.is_admin_approved).map(item => ({
    ...item,
    children: item.children?.filter(child => !child.verifiedOnly || user?.is_admin_approved)
  }));

  const mainNavItems = filteredMenu.slice(0, 4);
  const moreMenuItems = filteredMenu.slice(4);
  ...
  const renderMenuItem = (item: MenuItem, depth = 0) => {
    ...
    if (hasChildren) {
      return (
        <Collapsible key={item.href} open={isOpen} onOpenChange={() => toggleSection(sectionKey)}>
          <CollapsibleTrigger asChild>
            <button className={cn("flex w-full items-center justify-between rounded-lg px-3 py-3 text-sm font-medium transition-colors", isActive ? "bg-primary text-primary-foreground" : "hover:bg-accent hover:text-accent-foreground")}>
              <div className="flex items-center gap-3">
                <Icon className="h-4 w-4" />
                {item.label}
              </div>
              <ChevronUp className={cn("h-4 w-4 transition-transform", isOpen ? "rotate-180" : "rotate-0")} />
            </button>
          </CollapsibleTrigger>
          <CollapsibleContent className={cn("space-y-1", depth === 0 ? "ml-4 pl-4 border-l border-border" : "ml-6")}>
            {item.children?.map((child) => renderMenuItem(child, depth + 1))}
    ...
    return (
      <button
        ...
        className={cn("flex w-full items-center gap-3 rounded-lg px-3 py-3 text-sm transition-colors text-left", isActive ? "bg-primary/10 text-primary font-medium" : "hover:bg-accent hover:text-accent-foreground")}
      >
        <Icon className="h-4 w-4" />
        {item.label}
      </button>
  ...
  return (
    <>
      <nav className="fixed inset-x-0 bottom-0 z-50 border-t border-border bg-card lg:hidden pb-safe">
        <div className="flex items-center justify-around">
          {mainNavItems.map((item) => {
            const Icon = item.icon;
            // Precise active check for Dashboard specifically
            ...
            return (
              <button
                key={item.href}
                onClick={() => handleItemClick(item)}
                className={cn("flex flex-1 flex-col items-center gap-1 py-3 text-xs transition-colors", isActive ? "text-primary" : "text-muted-foreground hover:text-foreground")}
              >
                <Icon className={cn("h-5 w-5", isActive && "text-primary")} />
          ...
          {moreMenuItems.length > 0 && (
            <Sheet open={openMenu === "more"} onOpenChange={(open) => setOpenMenu(open ? "more" : null)}>
              <SheetTrigger asChild>
                <button className={cn("flex flex-1 flex-col items-center gap-1 py-3 text-xs transition-colors", openMenu === "more" ? "text-primary" : "text-muted-foreground hover:text-foreground")}>
                  <div className="flex space-x-0.5 h-5 items-center">
                    <div className="h-1 w-1 rounded-full bg-current"></div>
                    <div className="h-1 w-1 rounded-full bg-current"></div>
                    <div className="h-1 w-1 rounded-full bg-current"></div>
                  </div>
      ...
      {/* Sheets for main items with children */}
      {mainNavItems.map((item) => (
