...
//             variant="outline"
//             size="icon"
//             onClick={() => router.push(`/dashboard/professional/own-services`)}
//           >
  ...
  const loadData = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: getLocalizedText('Failed to load data', 'डाटा लोड गर्न असफल'),
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  const handleSaveServiceWithPrice = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: getLocalizedText('Service added successfully', 'सेवा सफलतापूर्वक थपियो'),
      });
      ...
      toast({
        title: getLocalizedText('Error', 'त्रुटि'),
        description: getLocalizedText('Failed to add service', 'सेवा थप्न असफल'),
        variant: 'destructive',
      });
    } finally {
      setSubmitting(false);
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              {getLocalizedText(
                'Add new services to your offerings',
                'तपाईंको सेवाहरूमा नयाँ सेवाहरू थप्नुहोस्'
              )}
            </p>
          </div>
        </div>
        ...
        <CardContent className="pt-6">
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1">
              <div className="relative">
                <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                <Input
                  placeholder={getLocalizedText('Search services...', 'सेवाहरू खोज्नुहोस्...')}
                  className="pl-10"
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                />
              ...
              <Popover open={openCategory} onOpenChange={setOpenCategory}>
                <PopoverTrigger asChild>
                  ...
                  >
                    <span className="truncate">
                      {selectedCategory 
                        ? truncateText(language === 'ne' ? selectedCategory.name_np || selectedCategory.name_en : selectedCategory.name_en, 25) 
                        : getLocalizedText('Select category...', 'श्रेणी चयन गर्नुहोस्...')}
                    </span>
                    <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                  </Button>
                </PopoverTrigger>
                <PopoverContent className="w-[300px] p-0">
                  <Command>
                    <CommandInput placeholder={getLocalizedText('Search category...', 'श्रेणी खोज्नुहोस्...')} />
                    <CommandList>
                      <CommandEmpty>{getLocalizedText('No category found.', 'कुनै श्रेणी फेला परेन।')}</CommandEmpty>
                      <CommandGroup>
                        {categories.map((category) => (
                          <CommandItem
                            key={category.id}
                            value={language === 'ne' ? category.name_np || category.name_en : category.name_en}
                            onSelect={() => {
                              setSelectedCategory(
                                category.id === selectedCategory?.id ? null : category
                              );
                          ...
                          >
                            ...
                            />
                            <span className="truncate" title={language === 'ne' ? category.name_np || category.name_en : category.name_en}>
                              {language === 'ne' ? category.name_np || category.name_en : category.name_en}
                            </span>
                          </CommandItem>
                        ))}
                      </CommandGroup>
              ...
              <Popover open={openSubCategory} onOpenChange={setOpenSubCategory}>
                <PopoverTrigger asChild>
                  ...
                  >
                    <span className="truncate">
                      {selectedSubCategory 
                        ? truncateText(language === 'ne' ? selectedSubCategory.name_np || selectedSubCategory.name_en : selectedSubCategory.name_en, 25) 
                        : getLocalizedText('Select sub-category...', 'उप-श्रेणी चयन गर्नुहोस्...')}
                    </span>
                    <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                  </Button>
                </PopoverTrigger>
                <PopoverContent className="w-[300px] p-0">
                  <Command>
                    <CommandInput placeholder={getLocalizedText('Search sub-category...', 'उप-श्रेणी खोज्नुहोस्...')} />
                    <CommandList>
                      <CommandEmpty>{getLocalizedText('No sub-category found.', 'कुनै उप-श्रेणी फेला परेन।')}</CommandEmpty>
                      <CommandGroup>
                        {getSubCategoriesForCategory().map((subCategory) => (
                          <CommandItem
                            key={subCategory.id}
                            value={language === 'ne' ? subCategory.name_np || subCategory.name_en : subCategory.name_en}
                            onSelect={() => {
                              setSelectedSubCategory(
                                subCategory.id === selectedSubCategory?.id ? null : subCategory
                              );
                          ...
                          >
                            ...
                            />
                            <span className="truncate" title={language === 'ne' ? subCategory.name_np || subCategory.name_en : subCategory.name_en}>
                              {language === 'ne' ? subCategory.name_np || subCategory.name_en : subCategory.name_en}
                            </span>
                          </CommandItem>
                        ))}
                      </CommandGroup>
          ...
          {(selectedCategory || selectedSubCategory) && (
            <div className="flex flex-wrap gap-2 mt-4">
              {selectedCategory && (
                <Badge variant="secondary" className="gap-1 max-w-full">
                  <span className="truncate" title={language === 'ne' ? selectedCategory.name_np || selectedCategory.name_en : selectedCategory.name_en}>
                    {getLocalizedText('Category', 'श्रेणी')}: {language === 'ne' ? selectedCategory.name_np || selectedCategory.name_en : selectedCategory.name_en}
                  </span>
                  <Button
                    variant="ghost"
                    size="sm"
              ...
              {selectedSubCategory && (
                <Badge variant="secondary" className="gap-1 max-w-full">
                  <span className="truncate" title={language === 'ne' ? selectedSubCategory.name_np || selectedSubCategory.name_en : selectedSubCategory.name_en}>
                    {getLocalizedText('Sub-category', 'उप-श्रेणी')}: {language === 'ne' ? selectedSubCategory.name_np || selectedSubCategory.name_en : selectedSubCategory.name_en}
                  </span>
                  <Button
                    variant="ghost"
                    size="sm"
            ...
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground mb-6">
                {searchQuery || selectedCategory || selectedSubCategory
                  ? getLocalizedText(
                      'Try changing your search or filters',
                      'कृपया आफ्नो खोज वा फिल्टर परिवर्तन गर्नुहोस्'
                    )
                  : getLocalizedText(
                      'No more services available to add',
                      'थप्नको लागि कुनै सेवा उपलब्ध छैन'
                    )}
              </p>
              {(searchQuery || selectedCategory || selectedSubCategory) && (
                <Button variant="outline" onClick={clearFilters}>
              ...
              <Card key={service.id} className="overflow-hidden hover:shadow-lg transition-shadow">
                <div className="relative h-48 bg-muted">
                  {service.image ? (
                    <Image
                      src={service.image}
                      alt={language === 'ne' ? service.name_np || service.name_en : service.name_en}
                      fill
                      className="object-cover"
                      sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw"
                    />
                ...
                <CardHeader>
                  <CardTitle className="line-clamp-1" title={language === 'ne' ? service.name_np || service.name_en : service.name_en}>
                    {language === 'ne' ? service.name_np || service.name_en : service.name_en}
                  </CardTitle>
                  <CardDescription className="line-clamp-2" title={language === 'ne' ? service.description_np || service.description_en : service.description_en}>
                    {language === 'ne' ? service.description_np || service.description_en : service.description_en}
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-2">
                    ...
                    {subCategory && (
                      <div className="text-sm text-muted-foreground truncate" title={language === 'ne' ? subCategory.name_np || subCategory.name_en : subCategory.name_en}>
                        {language === 'ne' ? subCategory.name_np || subCategory.name_en : subCategory.name_en}
                      </div>
                    )}
                  </div>
                ...
                <CardFooter>
                  ...
                  >
                    <Plus className="mr-2 h-4 w-4" />
                    {getLocalizedText('Add Service', 'सेवा थप्नुहोस्')}
                  </Button>
                </CardFooter>
              </Card>
            );
      ...
      if (val.length <= 2) {
       setDiscountPercentage(e.target.value)}
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSaveServiceWithPrice} disabled={submitting}>
              {submitting && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
              {getLocalizedText('Save & Add Service', 'सुरक्षित गर्नुहोस् र सेवा थप्नुहोस्')}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
